...
export function generateBeamData(name: string, colors: string[]): BeamData {
  ...
  return {
    ...
    maskId: `tronche-mask-beam-${numFromName}`,
  };
}

export function renderBeamSvg(data: BeamData, options?: { size?: number; square?: boolean; title?: string }): string {
  const displaySize = options?.size ?? SIZE;
  const square = options?.square ?? false;
  const rx = square ? '' : `rx="${SIZE * 2}"`;

  const wrapperTransform = `translate(${data.wrapperTranslateX} ${data.wrapperTranslateY}) rotate(${data.wrapperRotate} ${SIZE / 2} ${SIZE / 2}) scale(${data.wrapperScale})`;
...
<rect x="0" y="0" width="${SIZE}" height="${SIZE}" rx="${data.isCircle ? SIZE : SIZE / 6}" fill="${data.wrapperColor}" transform="${wrapperTransform}"/>
...
export function generateBeamSvg(name: string, colors: string[], options?: { square?: boolean; size?: number; title?: string }): string {
  const data = generateBeamData(name, colors);
  return renderBeamSvg(data, options);
}
