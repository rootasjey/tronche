  ...
  <div class="max-w-240 mx-auto px-5 pt-10 pb-15">
    ...
    <div class="bg-surface border border-[#1e1e22] rounded-xl p-6">
      ...
      <div v-if="showCreate" class="flex gap-2 flex-wrap mb-4">
        <input v-model="newKeyName" type="text" placeholder="Nom de la clé (ex: production, dev...)" class="flex-1 min-w-50 px-3.5 py-2 rounded-xl bg-[#0a0a0b] border border-[#1e1e22] text-white text-sm outline-none focus:border-primary" @keyup.enter="createKey" />
        <button class="inline-flex items-center px-4 py-2 rounded-full bg-primary text-white font-semibold text-xs border-none cursor-pointer hover:bg-primary-600 transition-colors" @click="createKey">Créer</button>
        <button class="inline-flex items-center px-4 py-2 rounded-full border border-[#1e1e22] text-white font-semibold text-xs bg-transparent cursor-pointer hover:border-white transition-colors" @click="showCreate = false">Annuler</button>
      </div>
